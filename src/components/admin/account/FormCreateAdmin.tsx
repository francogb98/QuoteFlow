...
import { createAdmin } from "@/actions/admin/account/createAdmin";
...
interface FormValues {
  ...
  documento: string;
  telefono: string;
  // â­ï¸ Nuevos campos del formulario
...
export const FormCreateAdmin = ({ setOpen }: FormCreateAdminProps) => {
  const {
    ...
    // â­ï¸ Establece valores por defecto para los checkboxes
    defaultValues: {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    // â­ï¸ Envía los nuevos campos al backend
    crear.mutate({
      email: data.email,
      nombre: data.nombre,
  ...
  return (
    <div className="p-4 border rounded bg-white">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2">
        ...
        {/* â­ï¸ Aquí van los nuevos checkboxes */}
