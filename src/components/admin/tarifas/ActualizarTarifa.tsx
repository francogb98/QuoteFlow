...
import { PlusIcon, TrashIcon, Save, Loader2, Edit } from "lucide-react";
...
interface ActualizarTarifaFormProps {
  existingConfig: any;
  onSuccess?: () => void;
}
...
export function ActualizarTarifaForm({
  existingConfig,
  onSuccess,
}: ActualizarTarifaFormProps) {
  const [tipoConfiguracion, setTipoConfiguracion] =
    useState<TipoConfiguracionTarifa>(
  ...
  const [autocompletadoNombre, setAutocompletadoNombre] = useState<string[]>(
    []
  );
  ...
  useEffect(() => {
    if (existingConfig) {
      ...
      if (
        ...
        const nuevosRangos = existingConfig.rangos
          .map((rango: any) => ({
            id: rango.id,
        ...
        setAutocompletadoNombre(nuevosRangos.map((r: any) => r.nombre));
        setDinamicas([]);
      } else if (
        existingConfig.tipoConfiguracion === "DINAMICA_POR_FECHA_INGRESO" &&
        existingConfig.dinamicas
      ) {
        setDinamicas(
          existingConfig.dinamicas.map((d: any) => ({
            id: d.id,
            tempId: crypto.randomUUID(),
            nombre: d.nombre,
        ...
        // Inicializar rangos vacíos para este tipo
        setRangos([]);
      }
    }
  }, [existingConfig]);

  // Limpiar alertas al cambiar tipo
  useEffect(() => {
    setAlert(null);
    setValidationErrors([]);
  }, [tipoConfiguracion]);
  ...
  const validateForm = (): string[] => {
    ...
    if (tipoConfiguracion === "FIJA_MENSUAL") {
      ...
      // Verificar solapamientos solo si el nombre es el mismo
      for (let i = 0; i < rangos.length; i++) {
        for (let j = i + 1; j < rangos.length; j++) {
          const a = rangos[i];
          const b = rangos[j];
          if (
            a.nombre.trim().toLowerCase() === b.nombre.trim().toLowerCase() && // Solo si el nombre es igual
            Number(a.diaInicio) <= Number(b.diaFin) &&
            Number(a.diaFin) >= Number(b.diaInicio)
          ) {
            errors.push(
              `Los rangos ${i + 1} y ${j + 1} se superponen y tienen el mismo nombre`
            );
          }
        }
      }
  ...
  const handleDinamicaChange = (
    tempId: string,
    field: keyof Omit<DinamicaTarifaForm, "id" | "tempId">,
    value: string
  ) => {
    setDinamicas(
      dinamicas.map((d) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await actualizarConfiguracionTarifa(configData);
                ...
                {rangos.map((r, i) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                      {/* hacer un input que autcomoplete con nombre si ya existe */}
                      ...
                      <div className="space-y-2">
                        ...
                        <datalist id="nombres-existentes">
                          {autocompletadoNombre.map((nombre, i) => {
                            return <option key={i} value={nombre} />;
                          })}
                        </datalist>
                      </div>
                      <div className="space-y-2">
