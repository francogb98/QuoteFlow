...
export function TariffManagement({ user }: TariffManagementProps) {
  ...
  const handleSave = async (data: TariffData) => {
    ...
    if (!hasConfiguracion) {
      ...
      result = await crearConfiguracionTarifa(payload as Parameters<typeof crearConfiguracionTarifa>[0]);
    } else {
      ...
      result = await actualizarConfiguracionTarifa(payload as Parameters<typeof actualizarConfiguracionTarifa>[0]);
    }
