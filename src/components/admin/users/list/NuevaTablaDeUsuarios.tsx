...
interface Usuario {
  ...
  pagos: {
    mes: number;
    año: number;
    estado: "PAGADO" | "PENDIENTE" | "INACTIVO" | "VENCIDO" | "RECHAZADO";
  }[];
}
...
export function NuevaTablaDeUsuarios({ profesorId }: Props) {
  ...
  const handleExportCsv = () => {
    const monthName = format(new Date(filterYear, filterMonth - 1), "MMMM", {
      locale: es,
    });
    const rows = sortedUsers.map((user) => ({
      ...
      Estado: getUserStatus(user),
    }));
    downloadCsv(rows, `usuarios_${monthName}_${filterYear}.csv`);
  };
        ...
        <div className="bg-gradient-to-r from-purple-50 to-emerald-50 p-4 sm:p-6 border-b border-purple-100 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-gray-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300"
