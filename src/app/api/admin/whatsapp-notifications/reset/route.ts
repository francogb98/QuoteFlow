...
import { resetWhatsAppNotificationStatus } from "@/lib/data/super-admin-whatsapp";
...
export async function POST(request: Request) {
  ...
  try {
    await resetWhatsAppNotificationStatus({
      usuarioId,
      pagoId,
    ...
    errorUrl.searchParams.set(
      "actionError",
      error instanceof Error ? error.message : "No se pudo resetear el estado",
    );
    return NextResponse.redirect(errorUrl);
  }

  const successUrl = new URL(returnTo, request.url);
  successUrl.searchParams.set("actionSuccess", "reset");
